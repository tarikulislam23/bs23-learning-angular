...
export declare interface FormErrorMessage {
  [key: string]: [];
}
...
export class FormErrorHandler {
  ...
  private errorObject: FormErrorMessage;
  private errorMessages: any;
  private message: string = "";
  private onFormSubmit: boolean = false;
   ...
   */
  public handleErrors(form: FormGroup, errorObject: any, givenErrorMessages: any = {}) {
    this.form = form;
    this.errorObject = errorObject;
    this.errorMessages = givenErrorMessages;
    ...
    for (const formGroup of formArray.controls as FormGroup[]) {
      ...
      Object.keys(controls).forEach(control => {
        if (FormErrorHandler.hasError(controls[control])) {
          this.setDefaultErrorMessage(controls[control].errors);
  ...
  private findErrorsOnFormControls(controls: { [key: string]: AbstractControl }, control: string) {
    ...
    if (FormErrorHandler.hasError(controls[control]) || this.onFormSubmit) {
      this.setGivenErrorMessage(controls[control]?.errors, control);
      this.setErrorToErrorObject(control);
    } else {
      this.message = "";
      this.setErrorToErrorObject(control); //Set empty error message if form control is valid
    }
  }

  private setGivenErrorMessage(errors: ValidationErrors, controlName: string) {

    console.log(errors);

    let msgSet: boolean = false;

    if (controlName in this.errorMessages && errors) {
      Object.keys(errors).forEach(validationName => {
        if (validationName in this.errorMessages[controlName]) {
          this.message = this.errorMessages[controlName][validationName];
          msgSet = true;
        }
      });
    }
    ...
    if (!msgSet)
      this.setDefaultErrorMessage(errors);
  }
   ...
   */
  private setDefaultErrorMessage(errors: ValidationErrors) {
    ...
    if (errors.serverError) {
      ...
      this.message = `Minimum length is ${errors.minlength.actualLength}/${errors.minlength.requiredLength}`;
    } else if (errors.maxlength) {
      this.message = `Maximum length is ${errors.maxlength.actualLength}/${errors.maxlength.requiredLength}`;
    ...
    } else if (errors.valueMoreThanZero) {
      this.message = 'Enter more than zero';
    } else {
      this.message = '';
    }
  }
